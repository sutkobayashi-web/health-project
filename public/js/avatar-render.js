...
function drawHair(ctx, cx, faceY, faceR, type, color) {
  ...
  switch(type) {
    case 1: // ショート
      ...
      hairCapFill(cx, topY + faceR * 0.1, faceR * 0.65, null, Math.PI, 2 * Math.PI);
    ...
    case 17: // ベリーショート — 坊主に近い超短髪
      ...
      hairCapFill(cx, topY + faceR * 0.1, faceR * 0.58, null, Math.PI, 2 * Math.PI);
    ...
    case 24: // 三つ編み — braids hanging down sides
      // 頭頂部ベース
      ctx.fillStyle = hairGrad(topY - faceR * 0.3, topY + faceR * 0.4);
      hairCapFill(cx, topY + faceR * 0.08, faceR * 0.85, null, Math.PI * 0.75, Math.PI * 2.25);
      // 分け目ライン
      if (detail) {
        ctx.strokeStyle = darker;
        ctx.lineWidth = Math.max(0.5, faceR * 0.015);
