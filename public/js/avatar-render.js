...
// 化粧パラメータ
var AB_LIP_COLORS = ['#D4636A','#D4357A','#E01030','#F06848','#7A1845','#C89080'];
var AB_LIP_COLOR_NAMES = ['ナチュラル','ローズ','レッド','コーラル','ベリー','ヌード'];
var AB_EYESHADOW_COLORS = [null,'rgba(230,130,170,0.5)','rgba(170,110,80,0.5)','rgba(150,100,200,0.55)','rgba(90,140,220,0.5)','rgba(210,170,70,0.5)'];
var AB_EYESHADOW_NAMES = ['なし','ピンク','ブラウン','パープル','ブルー','ゴールド'];
var AB_LASH_NAMES = ['ナチュラル','ロング','ボリューム'];
var AB_CHEEK_COLORS = ['rgba(255,100,150,','rgba(255,130,100,','rgba(210,80,130,','rgba(245,140,80,'];
...
var AvatarAssets = {
  ...
  async preloadAll() {
    ...
    this.isLoading = true;
    var promises = [];
    for (var type in this.partsConfig) {
      for (var i = 0; i < this.partsConfig[type]; i++) {
        var key = type + '_' + i;
        var src = '/img/avatar/' + key + '.png';
    ...
    if (this.isLoaded) {
      console.log('Avatar montage assets loaded (' + loadedCount + ' parts)');
      _avatarCache = {};
      document.dispatchEvent(new Event('avatarAssetsLoaded'));
    } else {
      console.log('Avatar montage assets not found, using legacy renderer');
    }
  },
  loadImage: function(key, src) {
    ...
    return new Promise(function(resolve) {
      var img = new Image();
      img.onload = function() { self.images[key] = img; resolve(img); };
      img.onerror = function() { self.images[key] = null; resolve(null); };
      img.src = src;
    });
  }
};

function getTintedImage(imageObj, colorStr) {
  if (!imageObj) return null;
  if (!colorStr || typeof colorStr !== 'string') return imageObj;
...
function renderMontageAvatar(avatarStr, size) {
  var cacheKey = 'montage_' + avatarStr + '_' + size;
  if (_avatarCache[cacheKey]) return _avatarCache[cacheKey];

  var originalState = window.abState ? JSON.parse(JSON.stringify(window.abState)) : null;
  if (typeof abParseString === 'function') abParseString(avatarStr);
  var state = window.abState || {};
  ...
  finalCanvas.width = retinaSize; finalCanvas.height = retinaSize;
  var finalCtx = finalCanvas.getContext('2d');
  finalCtx.imageSmoothingEnabled = true;
  finalCtx.imageSmoothingQuality = 'high';
  finalCtx.drawImage(baseCanvas, 0, 0, 400, 400, 0, 0, retinaSize, retinaSize);

  var dataUrl = finalCanvas.toDataURL('image/png');
  _avatarCache[cacheKey] = dataUrl;

  if (originalState) window.abState = originalState;
  return dataUrl;
}

// パーツ画像の非同期プリロード開始
AvatarAssets.preloadAll();
...
// ====== 旧Canvas描画（フォールバック） ======
function renderLegacyAvatar(avatarStr, size) {
  if (!avatarStr || !avatarStr.startsWith('custom:')) return null;
...
function drawCheeks(ctx, cx, eyeY, eyeSpacing, faceR, type, colorIdx) {
  if (type === 0) return;
  var cheekY = eyeY + faceR * 0.2;
  var cheekX = eyeSpacing + faceR * 0.08;
  var cheekR = faceR * 0.16;
  var alpha = type === 1 ? 0.13 : 0.25;
