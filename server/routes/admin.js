...
router.get('/inbox', (req, res) => {
  try {
    ...
    const result = posts.map(r => {
      ...
      return [r.id, r.post_id, r.content, r.analysis, r.nickname, latestAvatar, likesArr.length, r.post_id, r.category, r.status, r.user_id, r.image_url, nurse, nutri, dateStr, chatCounts[r.post_id] || 0, isTarget, isPlanned, demotesArr.length, r.admin_read_at || null, r.nutrition_scores || null];
    });
    res.json(result);
  } catch (e) { res.json([]); }
});
...
router.post('/simulate-meeting', async (req, res) => {
  try {
    ...
    const sysPrompt = `あなたは中小運送会社の健康推進プランを評価する「AI評議会」です。
以下のプラン候補について、5名の専門家が順番に論点を整理してください。

【専門家メンバー】
1. AIメディカルアドバイザー(🩺) - 医学的妥当性、エビデンスの観点
2. AIヘルスアドバイザー(💉) - 産業保健、労働安全衛生の観点
3. AI食事アドバイザー(🥗) - 栄養学、食習慣改善の観点
4. AI経営アドバイザー(📊) - コスト対効果、経営インパクトの観点
5. AI現場アドバイザー(🚛) - ドライバーの実態、現場実現可能性の観点

【プラン】
タイトル: ${planData.title}
内容: ${planData.draft || planData.description || ''}

【各専門家の発言ルール】
- まず結論（賛成/条件付き賛成/要検討）を明示
- 論点を1〜2点に絞り、具体的に指摘（各80〜120字）
- 改善提案があれば簡潔に付記
- ドライバーの生活実態（長時間運転、コンビニ食中心、不規則な生活）を踏まえる
