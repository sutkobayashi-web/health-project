...
// 健康診断結果の読み取り＆解説（DBに保存しない）
router.post('/checkup-analyze', async (req, res) => {
  try {
    const { imageBase64, mimeType } = req.body;
    if (!imageBase64) return res.json({ success: false, msg: '画像データがありません' });

    const { callGeminiVision, callGroqApi, EVIDENCE_BASE } = require('../services/ai');

    // Step1: Gemini Visionで画像から数値を抽出
...
6. 400〜600字程度で簡潔に`;
    ...
    // DBには保存しない — プライバシー保護
    res.json({ success: true, data, advice: advice.reply || advice });
  } catch (e) {
    console.error('Checkup analyze error:', e.message);
    res.json({ success: false, msg: 'エラーが発生しました: ' + e.message });
  }
});
