...
router.get('/daily-greeting', async (req, res) => {
  try {
    ...
    const prompt = `今日は${d.getFullYear()}年${month}月${day}日（${weekday}曜日）です。${weatherInstruction}
...
{
  "dateInfo": "エイプリルフール 🤡",
  "dateFact": "1564年フランスの暦改正が起源とされる説が有名",
...
{
  "dateInfo": "ありがとうの日 💐",
  "dateFact": "3(サン)9(キュー)の語呂合わせで感謝を伝える日",
  "healthTip": "🚶 春のぽかぽか陽気！昼休みに10分だけ外を歩いてみよう"
}

【あなたが返すJSON】
{
  "dateInfo": "${month}月${day}日の記念日・語呂合わせの日・歴史的出来事を1つ選び、絵文字1つ付きで20文字以内",
  "dateFact": "由来や背景を具体的に25〜35文字で説明",${weatherField}
  "healthTip": "「${todayTipCategory}」に関連する今日からできる具体的なアドバイスを絵文字1つ付きで35〜45文字。「〜してみよう」「〜がオススメ」調で"
}

【厳守ルール】
- ${month}月${day}日に実在する記念日のみ使用。架空の記念日を創作しない
- 各フィールドは具体的に書く。「春の到来」「体温調節に注意」のような抽象的な表現は禁止
- healthTipは食材名、運動名、数値（○分、○g等）を含める
- 有効なJSONのみ返す。コードブロックや説明文は不要`;

    const aiResult = await callAIWithFallback(
      `あなたは日本の記念日・歴史・暦に精通した百科事典です。${d.getFullYear()}年${month}月${day}日について正確で具体的な情報を返してください。不確かな情報や架空の記念日は絶対に返さないでください。`,
      prompt
    );
